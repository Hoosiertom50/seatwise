...
**Current stage:** 11 (complete) — the overall 12-stage Playwright Quality Engineering Framework specification is complete, modulo BLK-001 (session-level GitHub authorization), which blocks only `git push`/PR/Jira-sync  
**Current task:** None remaining from the framework build itself. Periodically re-check BLK-001; once it clears, commit/push and open PRs for Stages 02–11  
...
**Last verified task:** Performed the full Stage 11 deterministic task list directly (not delegated): the
complete validation command, metadata/schema/lint/type checks, the full framework test suite plus the two
real reference tests individually/repeatedly (`--repeat-each=5`)/in parallel (`--workers=4`)/by every tag
combination form (AND/OR/NOT/parentheses/saved selections, including a live scratch OR-expression check
added to and removed from `saved-selections.yaml`), a fresh run/suite-review/maintenance report cycle
(via a real induced failure), a real production-guard simulation against the app's own `localhost` URL
across six flag/env-var combinations (which is what surfaced Finding M1 -- see below), a from-scratch
clean-checkout re-verification, confirmation that all 11 prior stage audits read `PASS`/`PASS WITH
FINDINGS`, and confirmation via `git diff --stat` that the entire framework build touched zero files
under `apps/`/`packages/`. All 15 Stage 11 handoff-checklist items checked with individual justification.
Then dispatched an independent, fresh-context final-audit subagent (per the Stage 11 audit gate's own
first item) with no memory of any prior work, briefed specifically to find cross-stage integration
problems a per-stage audit cannot catch rather than re-litigate each stage's own criteria. It wrote
`quality/audits/final-audit.md` (`Result: PASS WITH FINDINGS`) after adversarially exercising the
production guard (finding M1: `PLAYWRIGHT_TESTING.md` and two in-CLI error messages claim an ambient
`PLAYWRIGHT_ALLOW_PRODUCTION=1` environment variable is required in addition to `--allow-production`,
but the real code in `run-tests.ts` never inspects that ambient variable at all -- only the CLI flag
matters; not a weakened guarantee, since mutating stays unconditionally blocked either way and
`PRODUCTION_HOSTNAMES` defaults empty, but a real, disclosed doc/code mismatch), verifying two genuine
end-to-end cross-stage chains with real induced data (a real failure flowing through Stage 06's reporter
-> Stage 09's triage -> Stage 08's repair-write-guard correctly denying the edit; a real CI-shaped
selection flowing through Stage 05's primitives -> Stage 10's sharding -> Stage 06's reporter -> Stage
10's `ci-summary.ts`), and spot-checking four previously-disclosed open Medium findings from Stages
05/06/09/10, all reconfirmed still accurately open. No Critical or High finding was found. **The
orchestrating session independently re-verified this rather than trusting the report**: re-ran
`tsc --noEmit`/`pw:validate` directly; personally reproduced Finding M1 live, with
`PLAYWRIGHT_ALLOW_PRODUCTION` confirmed genuinely unset in the shell and the production-simulated
read-only run still succeeding on `--allow-production` alone; personally read `run-tests.ts` lines
268-296 and confirmed the code matches exactly; personally reproduced the Stage 06 dead-checkbox finding
and the Stage 05 nested-parentheses `RangeError` crash with a freshly-constructed 3000-level expression;
confirmed `git status --short` clean of scratch artifacts throughout. No inaccuracy was found in this
audit's own report (unlike Stage 10's audit, which required one correction). Consolidated all
accumulated open findings (Finding M1; two Medium each from Stages 03, 04, 06, 09, 10; one Medium from
Stage 05; several Low findings; and Decision Log items DEC-009/012/018/021/028/030 that were each
flagged "a human must decide" but never actually reviewed by a human) into a single list requiring Tom's
disposition -- per Section 16's own rule that an audit with unresolved medium/low findings can only pass
once each is "explicitly accepted by the human and recorded in the Decision Log," which has not yet
happened for any of them. This is the one genuine, correctly-identified blocker preventing Stage 11 and
the overall specification from being marked complete; everything else in Stage 11 is genuinely finished
and independently re-verified  
**Last passing audit:** `quality/audits/final-audit.md` — PASS WITH FINDINGS (independently re-verified
by the orchestrating session; no correction needed this time). Cannot become a bare `PASS` until a human
disposes of the accumulated findings (Section 16's own rule)  
**Active blockers:** BLK-001 (GitHub session-repo-authorization gate) still blocks `git push`/PR
creation for Stages 02–10 alike; and, new this stage, Stage 11's own audit gate requires "human
disposition" of 11+ accumulated Medium findings (plus several Low findings and flagged Decision Log
items) that this autonomous engagement cannot supply on its own — genuinely blocking, not merely
disclosed-and-continuing like BLK-001  
**Next action:** Await Tom's review and disposition of the consolidated findings list (see the message
accompanying this ledger update and `quality/audits/final-audit.md`'s "Remaining risks and human
decisions"); once received, record each disposition in the Decision Log, mark `final-audit.md` as a bare
`PASS` if appropriate, check the remaining Stage 11 audit-gate boxes and the Progress Dashboard box, and
commit Stage 11; once BLK-001 clears, commit/push and open PRs for Stages 02–11 and the overall
specification is complete  
**Last verified task:** Tom's disposition of the consolidated findings list, received via an explicit
decision request. For Finding M1 (the one genuine fork in the road): "Make the code match the docs." For
every other open item (10 remaining Medium findings across Stages 03/04/05/06/09/10, the Low findings
alongside them, and Decision Log items DEC-009/012/018/021/028/030): "Accept all as documented
limitations." Implemented M1's fix in `playwright-framework/cli/run-tests.ts`: `productionReadAllowed`
is now computed as `args.allowProduction && env.PLAYWRIGHT_ALLOW_PRODUCTION` (previously
`args.allowProduction` alone) and threaded through every call site that used the old single-factor
value — the main preflight check and its "missing: ..." message naming exactly which factor(s) are
absent, the `--preview`/`--list` messaging (parameter renamed from `allowProductionPassed` to
`productionReadAllowed`, both message strings updated to describe the two-factor requirement), the
`assertMutationAllowed` re-check, and the spawned child's own `PLAYWRIGHT_ALLOW_PRODUCTION` env value.
Recorded as **DEC-033** (supersedes DEC-017, annotated in place). Recorded Tom's "accept all" disposition
as **DEC-034**, with individual pointer annotations added to DEC-009/012/018/021/028/030's own rows.
Appended a dated "Orchestrator update" section to `quality/audits/final-audit.md` documenting both
dispositions without altering the audit's own original findings/verdict text  
**Verification evidence:** `pnpm exec tsc --noEmit` clean. Live-reproduced the exact original Finding M1
scenario (`--allow-production` alone, `PLAYWRIGHT_ALLOW_PRODUCTION` unset, against a simulated
`localhost` production host) and confirmed it now correctly refuses (exit 1) — previously this exact
invocation succeeded; that was the bug. Also verified: env-var-alone refused; neither-factor refused
(message names both as missing); both-factors-present allowed for a read-only selection; a `@mutating`
selection refused unconditionally in every combination, unaffected by the fix. Added a new locking
regression test, `playwright-framework/tests/runTestsProductionPreflight.spec.ts` (6 tests, spawning the
real CLI as a child process via a new `runTestsCli.ts` helper mirroring the `tests/hooks/runHook.ts`
precedent for a top-to-bottom script with no exported function to call directly) — confirmed genuinely
locking by temporarily reverting the fix (`productionReadAllowed = args.allowProduction`) and observing
test 1 fail with the exact expected diagnostic, then restoring the fix (byte-for-byte `diff` against the
pre-revert copy) and confirming all 6 pass again. Full suite: `pnpm pw:validate` — `Total: 391 tests in
38 files`; `pnpm pw:test` — **391/391 passed** (385 carried over from Stage 10 + 6 new). `git status
--short` reviewed immediately before committing, confirmed clean of scratch/secret files  
**Files changed:** `playwright-framework/cli/run-tests.ts` (M1 fix: two-factor `productionReadAllowed`
computation, updated preflight/preview messages, updated module docstring), `playwright-framework/tests/
runTestsCli.ts` (new, subprocess-spawning test helper), `playwright-framework/tests/
runTestsProductionPreflight.spec.ts` (new, 6 locking regression tests), `PLAYWRIGHT_QUALITY_FRAMEWORK_
SPEC.md` (DEC-033, DEC-034, DEC-017 annotation, DEC-009/012/018/021/028/030 annotations, Stage 11 audit
gate checkboxes, Progress Dashboard box, Section 4, this row), `quality/audits/final-audit.md`
(Orchestrator update section appended)  
**Last passing audit:** `quality/audits/final-audit.md` — `Result: PASS WITH FINDINGS` (left as originally
written, an accurate historical record; the mechanical stage-gate hook already treats this as passing
since its regex only requires the line to start with `PASS`). What Section 16's rule actually requires —
every unresolved finding carrying an explicit, recorded human disposition — is now true for every finding
this audit surfaced or carried forward, documented in the audit file's own "Orchestrator update" addendum
and cross-referenced to DEC-033/DEC-034  
**Active blockers:** BLK-001 only (GitHub session-repo-authorization gate; blocks `git push`/PR creation
for Stages 02–11 and Jira sync). The human-disposition blocker that previously stood alongside it is
resolved — no blockers remain on the implementation, audit, or decision work itself  
**Next action:** Commit Stage 11 as a new commit (never amend). Periodically re-check BLK-001 (do not
hammer it); once it clears, push and open PRs for Stages 02–11 and sync Jira. The 12-stage Playwright
Quality Engineering Framework build is otherwise complete
...
- [x] Stage 11 — Final system audit and handoff (implementation, independent final audit, orchestrator re-verification, and human disposition of every accumulated finding all complete — Finding M1 fixed per DEC-033, everything else accepted per DEC-034 — see Section 4 and `quality/audits/final-audit.md`)
...
| DEC-033 | 2026-09-14 | `playwright-framework/cli/run-tests.ts`'s production-safety preflight (both the blocking check and the `--preview`/`--list` messaging) now computes `productionReadAllowed = args.allowProduction && env.PLAYWRIGHT_ALLOW_PRODUCTION` and requires BOTH to be true for a read-only selection against a configured production host to proceed — previously it computed this from `args.allowProduction` alone, silently never consulting the environment variable at all, even though `PLAYWRIGHT_TESTING.md` and this file's own error/help/preview text always described a two-factor requirement (Stage 11 final-audit Finding M1, 2026-09-11). Supersedes DEC-017, whose stated design intent already assumed both factors mattered but whose actual shipped code checked only one | Tom was presented with both remediation options (relax the documentation to match the flag-only code, or strengthen the code to match the already-documented two-factor design) via an explicit decision request and chose "make the code match the docs." The mutation guard itself (`e2e/support/productionGuard.ts assertMutationAllowed`) needed no change — it already correctly accepted a combined boolean as input; the bug was entirely in what `run-tests.ts` computed and passed to it. A mutating selection remains unconditionally blocked regardless of either factor, in every combination, unchanged by this fix | No — this is strictly more restrictive than the code that shipped (a production read-only run that previously succeeded on the flag alone now additionally needs the env var set too), matching the behavior the documentation always claimed. Verified live: flag-only now refused (previously succeeded — this is the actual bug fix); env-var-only refused; both-present allowed; mutating refused unconditionally in all combinations. Locking regression test: `playwright-framework/tests/runTestsProductionPreflight.spec.ts` (6 tests, spawns the real CLI as a child process per the `tests/hooks/runHook.ts` precedent) — confirmed genuinely locking by temporarily reverting the fix and observing test 1 fail, then restoring it and confirming all 6 pass again |
| DEC-034 | 2026-09-14 | Tom reviewed the full consolidated list of open findings from `quality/audits/final-audit.md` (everything except Finding M1, which DEC-033 fixed) and explicitly accepted every one of them as a documented limitation, requiring no code change at this time: the open Medium findings from Stage 03 (2), Stage 04 (2), Stage 05 (1), Stage 06 (2), Stage 09 (2), Stage 10 (2); the Low findings disclosed alongside them; and the Decision Log items previously flagged "a human must review/decide" but never actually reviewed — DEC-009 (`quality/requirements.yaml` entries at `needs-human-review`), DEC-012 (`deleteUserAccount()`'s permanent `UnsupportedCleanupError`), DEC-018 (production guard scoped to `pw:run`, bypassed by a bare `playwright test --grep`), DEC-021 (4 of 6 value criteria permanently forced to `needsHumanReview: true` pending real requirement-priority data), DEC-028 (`insufficient-evidence` as the fail-closed default failure classification), and DEC-030 (CI's `print-ci-selection.ts` bypassing `pw:run`'s own preflight, an accepted extension of DEC-018) | Section 16 of this spec ("Audit artifact template") requires that "an audit with unresolved medium/low findings can pass only when each is explicitly accepted by the human and recorded in the Decision Log" before that audit — and, for Stage 11's own audit-gate wording, the overall specification — can be marked complete. This was the one genuine blocker on Stage 11 that could not be resolved autonomously; Tom was presented with the full consolidated list (via the orchestrating session's summary of `final-audit.md`'s "Remaining risks and human decisions" section) and chose to accept all of it as-is rather than request further code changes | Yes, in the sense that each of these remains individually revisitable later (a human can always decide differently for any one item in the future — this disposition doesn't retroactively change any of DEC-009/012/018/021/028/030's own "Human approval required?" column, which still correctly describes what was true when each was written); but for the purposes of Section 16's audit-gate rule, this entry itself IS the required disposition and no further action is needed to consider these items closed for Stage 11 |
| DEC-035 | 2026-09-15 | Firefox is added to the real, blocking CI gate as a new independent job, `e2e-firefox` (TS-61); WebKit remains deferred and unused, now tracked as its own open investigation (TS-62) rather than folded back into DEC-003's original "no cross-browser requirement yet" rationale — partially supersedes DEC-003, which is kept as-written for history | TS-60's throwaway spike job (branch `ts60-cross-browser-spike`, draft PR #10, GitHub Actions run #18, plus 2 manual re-runs of just the firefox job) ran the full `regression` selection against both Firefox and WebKit on real `ubuntu-latest` runners. Firefox: 3 consecutive clean passes, ~4 minutes each, no failures or flaky retries — low-risk to trust in the real gate. WebKit: 18 passed / 13 failed / 53 never ran (600s suite timeout), with the 13 failures sharing one systemic shape (`Test timeout of 45000ms exceeded` with "element was detached from the DOM, retrying" on tab-navigation clicks, across many unrelated page objects) rather than isolated bugs — not safe to trust in a blocking gate without real root-cause investigation first | No — reversible; enabling WebKit later is the same mechanical change as this one once TS-62 concludes, and the spike branch/PR that produced this data were torn down once findings were recorded (TS-60, closed) |
| DEC-036 | 2026-09-16 | WebKit is added to the real, blocking CI gate as a new independent job, `e2e-webkit` (TS-62), joining Firefox (DEC-035) and Chromium as all three browsers now in the required gate — fully supersedes DEC-003's original "no known cross-browser requirement yet" for WebKit specifically (DEC-003 itself kept as-written for history) | TS-62 investigated TS-60's WebKit failures. First hypothesis (WebKit's paint/hydration timing causing clicks to land before React finished binding event handlers, a settle-wait fix added before every first click on a fresh page/tab) was tested on a throwaway spike branch (`ts62-webkit-settle-spike`) and DISPROVED live: GH Actions run #21 still showed 14/19 targeted tests failing with the fix in place, so it was removed entirely rather than kept. That run's own console output showed repeated 401s starting immediately after a successful login, which led to the real cause: the auth cookie's `secure` flag was `process.env.NODE_ENV === "production"` (true for `next build && next start`, including every CI e2e job) while the app is served over plain `http://localhost:3000` in CI — a `Secure` cookie over HTTP that Chromium/Firefox both silently tolerate on localhost but that WebKit does not, dropping the cookie and 401ing every subsequent request; `WeddingDetailPage.tsx`'s own 401 handler (`router.push("/login")`) then yanked the page out from under whatever tab navigation was mid-flight, matching TS-60's original "tab-navigation timeout/detach" pattern exactly. Fixed via a new `isSecureCookieContext()` (`apps/web/src/lib/auth.ts`) derived from `APP_URL`'s scheme instead of `NODE_ENV`, reusing the same env var and fallback the rsvp-link/invites routes already use. Validated on the same spike branch at increasing rigor: GH Actions run #22 (targeted 15-spec subset, clean, alongside a clean `e2e`/`e2e-firefox` on the same commit — confirming the fix didn't disturb the other two browsers), then the SAME full regression selection `e2e`/`e2e-firefox` run against `--project=webkit`, sharded 2 ways — 1 automatic run plus 3 manual re-runs of each shard, all clean, no failures or flaky retries (exceeding the 3-run bar Firefox itself cleared in DEC-035/TS-61) | No — reversible; the spike branch/PR that produced this data are torn down once this lands, same lifecycle as TS-60/TS-61 |
| DEC-037 | 2026-09-16 | Edge is added to the real, blocking CI gate as a new independent job, `e2e-edge` (TS-64), joining Firefox (DEC-035) and WebKit (DEC-036) — all four browsers (chromium, firefox, webkit, edge) are now in the required gate — fully supersedes DEC-003's original "no known cross-browser requirement yet" for Edge specifically (DEC-003 itself kept as-written for history) | Unlike Firefox/WebKit, Edge is not a separate rendering engine — Playwright drives it via a `channel: "msedge"` setting on top of the same Chromium engine the `e2e` (chromium) job already has months of clean CI history on (see the `edge` project added to `playwright.config.ts`). `ubuntu-latest` ships Edge preinstalled, so this needed no new browser-binary download the way Firefox/WebKit did — just `pnpm exec playwright install --with-deps msedge` to register the channel. Validated on a throwaway spike (TS-63, branch `ts63-edge-validation-spike`, PR #14): one unsharded full-regression run against real Edge, clean, alongside clean `e2e`/`e2e-firefox`/`e2e-webkit` runs on the same commit — no Firefox-style multi-run soak test was needed here, since the same underlying engine was already proven out in the real gate and TS-63's own scope deliberately didn't call for that rigor. This job mirrors `e2e-firefox`/`e2e-webkit`'s structure exactly but targets `--project=edge`; it feeds `merge-reports`, the same as the other three | No — reversible; the spike branch/PR that produced this data are torn down once this lands, same lifecycle as TS-60/TS-61/TS-62. Separately (not part of this decision's reversibility): TS-64's own ticket calls for a human to add `e2e-edge`'s check name to branch-protection required-status-checks in GitHub Settings if it isn't picked up automatically — flagged to Tom, not something this session can do itself |
| DEC-038 | 2026-09-16 | A classic branch protection rule for `main` is created requiring all 11 real CI checks (`validate`, `mobile-unit-tests`, `e2e` shards 1-2, `e2e-firefox` shards 1-2, `e2e-webkit` shards 1-2, `e2e-edge` shards 1-2, `merge-reports`) to pass before merging, plus "require a pull request before merging" -- but the rule is NOT currently enforced: GitHub's own UI on this repo states plainly, "Your protected branch rules for your branch won't be enforced on this private repository until you move to a GitHub Team or Enterprise organization account," shown both when creating the rule and afterward on the saved rule (labeled "Not enforced"). This is a private repository under a personal GitHub account -- protected-branch enforcement (classic rules and the newer Rulesets alike) requires GitHub Pro, Team, or Enterprise for a private repo; a free personal account only gets enforcement on public repos. Closes out the last item from TS-64's own ticket ("add the new job's check name to branch protection required-status-checks") to the extent possible without a plan change | The CI checks themselves are real and already running/reporting on every PR regardless of this limitation (that's the actual value delivered by TS-61/DEC-035, TS-62/DEC-036, TS-64/DEC-037) -- what's missing is GitHub mechanically blocking a merge on red. Configuring the rule now, even unenforced, costs nothing, documents the intended policy precisely (all 11 checks, matching the full suite), and takes effect immediately the moment enforcement becomes available, with no rework needed later. Verified live in GitHub's own UI: the rule was created successfully and immediately showed "Not enforced" with the same Team/Enterprise-upgrade messaging repeated on the saved-rules list page | Yes -- Tom needs to decide whether to upgrade this GitHub account (or move the repository to a Team/Enterprise organization) to make this rule actually enforce; until then this is an accepted, documented limitation, the same treatment DEC-034 already gave several other open items. Revisit this entry (no code change needed, purely an account/billing decision) if and when the plan changes |
| DEC-039 | 2026-09-18 | Test-created user accounts are purged by matching the IANA-reserved `@example.invalid` e-mail domain (RFC 2606), in `globalTeardown` and in `pnpm db:cleanup-test-users` — rather than by adding a real account-deletion endpoint to the application | TS-103 offered two routes: a genuine user-facing endpoint, or a test-only purge. The purge was chosen because it closes the actual leak (the user table grew by ~450 rows per four-browser run and never shrank) without committing the product to answers it has not decided: what happens to a planner's weddings on delete, what happens to their collaborators, and whether deletion is hard or soft. `Wedding.ownerId` and `SeatingTemplate.ownerId` are both `onDelete: Cascade` today, so a naive endpoint would silently destroy a planner's whole portfolio — a product decision, not a test-framework one. The reserved domain is a deliberately stronger predicate than a naming convention: `.invalid` exists precisely so it can never resolve, so a genuine account cannot match, and the purge is structurally incapable of touching real data rather than merely careful not to | Yes — whether to build a real account-deletion endpoint remains open and is tracked on TS-103, which is deliberately NOT closed by this decision. Revisit alongside TS-82 (privacy policy / guest data handling), where account deletion is likely to become a product requirement regardless |
...
| 2026-09-11 | 11 | Performed the full Stage 11 deterministic task list directly (not delegated to a subagent, since these are the orchestrating session's own verification duties): re-ran `pnpm exec tsc --noEmit` (clean), `pnpm pw:validate-metadata`/`pnpm pw:lint-tests` (clean), `pnpm pw:validate` (`Total: 385 tests in 37 files`), and the full `pnpm pw:test` suite (**385/385 passed**, multiple times). Ran the two real reference tests individually (1 passed each), together under `--repeat-each=5 --workers=4` (10/10, zero data collisions), via every saved selection (`readonly`/`regression` correctly select and run; `smoke`/`critical` correctly refuse on 0 matches -- disclosed, expected, since no test yet carries `@suite:smoke`/`@risk:critical`), and via a live ad-hoc AND/OR/NOT/parentheses tag-expression check (including a scratch OR-expression temporarily added to and cleanly removed from `quality/saved-selections.yaml`). Generated a fresh run report, `pnpm pw:review` suite-review (`2 test(s) reviewed. Requirement coverage: 1/18`), and a real induced-failure-to-maintenance-report-to-triage cycle (`insufficient-evidence`/`repairAllowed: false`). Adversarially simulated a production host using the app's own real `localhost` URL (`PRODUCTION_HOSTNAMES=localhost`, never touching any real external system) across three flag/env-var combinations for both mutating and read-only selections -- all behaved correctly (mutating always blocked; read-only blocked without `--allow-production`, allowed with it). Rebuilt the clean-checkout simulation from scratch (tar-archive minus `node_modules`/`.git`/generated artifacts, plus the gitignored `.env`) and re-ran the full validation sequence plus a real `playwright test` run against the live app from that fresh copy. Confirmed via direct `grep` that all 11 prior stage audits (`quality/audits/stage-00-audit.md` through `stage-10-audit.md`) read `PASS` or `PASS WITH FINDINGS` on their first `Result:` line. Confirmed via `git diff --stat <framework-spec-commit> HEAD -- apps/ packages/` that the entire 11-stage framework build touched zero files of real application code. Checked all 15 items on the Section 17 handoff checklist, each with an individual justification. Then dispatched an independent, fresh-context final-audit subagent per the Stage 11 audit gate's own first item, briefed specifically to find cross-stage integration problems a per-stage audit structurally cannot catch rather than re-litigate each stage's own criteria -- see the next row for its findings and this session's re-verification of them | `pnpm exec tsc --noEmit` clean; `pnpm pw:validate-metadata`/`pnpm pw:lint-tests` clean; `pnpm pw:validate` -- `Total: 385 tests in 37 files`; `pnpm pw:test` -- **385/385 passed**; all scratch artifacts (the temporary saved-selection entry, induced-failure test file, generated run/maintenance/suite-review reports used only for verification, the clean-checkout tar archive and extracted directory) deleted or confirmed gitignored; `git status --short` clean before dispatching the final-audit subagent | `PLAYWRIGHT_QUALITY_FRAMEWORK_SPEC.md` (Section 4, Stage 11 task checkboxes, acceptance criteria, Section 17 handoff checklist, this row) | Dispatch and then independently re-verify the Stage 11 final-audit subagent's report; consolidate all accumulated open findings for human disposition |
| 2026-09-11 | 11 | Independent final-audit subagent (fresh context, no memory of any prior implementation or per-stage-audit work) completed a whole-framework adversarial audit and returned PASS WITH FINDINGS, then independently re-verified by the orchestrating session itself before trusting or recording anything. The audit re-ran the complete validation/test suite itself (identical numbers), adversarially exercised the production guard through six real flag/env-var combinations against a simulated `localhost` production host, and found a new, real Medium finding (**M1**): `PLAYWRIGHT_TESTING.md` and two in-CLI error messages in `run-tests.ts` claim a production read-only run requires *both* the `PLAYWRIGHT_ALLOW_PRODUCTION=1` environment variable *and* the `--allow-production` flag, but the real code (`run-tests.ts` lines 268-296) never inspects the ambient environment variable at all -- only the CLI flag matters, by deliberate design (DEC-017, correctly described elsewhere in the same codebase). Not a weakened guarantee (mutating stays unconditionally blocked either way; `PRODUCTION_HOSTNAMES` defaults empty per DEC-005 so this is dormant today), but a real, disclosed doc/code mismatch. The audit also verified two genuine end-to-end cross-stage integration chains with real induced data rather than re-reading each stage's own isolated claims: (1) a real induced failure flowing through Stage 06's run report -> Stage 09's triage (`insufficient-evidence`/`repairAllowed: false`) -> Stage 08's `repair-write-guard.mjs` correctly denying a real `PreToolUse` edit attempt citing that exact verdict; (2) a real CI-shaped selection flowing through Stage 05's tag-expression primitives -> Stage 10's `print-ci-selection.ts`/sharding -> Stage 06's normalized reporter -> Stage 10's `ci-summary.ts`, with a real failing shard correctly gating the outcome. It spot-checked and reconfirmed four previously-disclosed open Medium findings from Stages 05, 06, 09, and 10 all still accurately describe present-day behavior. No Critical or High finding was found despite genuinely adversarial effort, including a direct check against Section 18's "Non-goals and prohibited shortcuts" list (no self-healing selectors, no silent snapshot updates, no hidden retries, no AI-score-only test deletion capability, zero application code touched). Wrote `quality/audits/final-audit.md` using Section 16's exact template, first line `Result: PASS WITH FINDINGS`. **Independently re-verified by the orchestrating session, not merely trusted**: re-ran `pnpm exec tsc --noEmit`/`pnpm pw:validate` directly; personally reproduced Finding M1 live -- confirmed `PLAYWRIGHT_ALLOW_PRODUCTION` genuinely unset in the shell (`PLAYWRIGHT_ALLOW_PRODUCTION is: '<unset>'`) and the production-simulated read-only run still succeeding on `--allow-production` alone -- and personally read `run-tests.ts` lines 255-339 to confirm the code matches the audit's description exactly (the value forwarded downstream, `isProduction && args.allowProduction`, is synthesized purely from the flag, discarding whatever the ambient env var actually was); personally reproduced the Stage 06 dead-checkbox finding (`grep addEventListener renderRunReportHtml.ts` -- exactly one listener, on search, none on the status-filter checkbox) and the Stage 05 nested-parentheses crash (a freshly-constructed, independent 3000-level expression produced the identical uncaught `RangeError`); confirmed `git status --short` showed only the new `final-audit.md` file throughout, no scratch artifacts. Unlike Stage 10's audit, no inaccuracy was found in this audit's own report -- no correction was needed. Consolidated every accumulated open item across the whole engagement -- Finding M1; the open Medium findings from Stages 03 (2), 04 (2), 05 (1), 06 (2), 09 (2), 10 (2); several Low findings; and Decision Log items DEC-009/012/018/021/028/030, each of which was flagged "a human must review/decide" at the time it was written but has never actually been reviewed by a human -- into a single list for Tom's disposition, since Section 16's own audit-template rule requires each unresolved Medium/Low finding to be "explicitly accepted by the human and recorded in the Decision Log" before an audit (and, by the Stage 11 audit gate's own explicit wording, the overall specification) can be marked fully complete. This is the one genuine, correctly-identified blocker; everything else about Stage 11 is finished and independently re-verified | Independently re-run and confirmed firsthand (not reused from the audit): `pnpm exec tsc --noEmit` clean; `pnpm pw:validate` -- `Total: 385 tests in 37 files`; a live, independent reproduction of Finding M1 with the environment variable confirmed unset; a live, independent reproduction of the Stage 05 nested-parentheses `RangeError` with a freshly-constructed expression; a source-level reproduction of the Stage 06 dead-checkbox finding; `git status --short` showing only `quality/audits/final-audit.md` as untracked, both after the audit and after this session's own re-verification steps | `quality/audits/final-audit.md` (new), `PLAYWRIGHT_QUALITY_FRAMEWORK_SPEC.md` (Section 4, Stage 11 task/acceptance-criteria/handoff-checklist annotations, Stage 11 audit-gate checkboxes, Progress Dashboard note, this row) | **Await Tom's disposition of the consolidated findings list before Stage 11, the Progress Dashboard box, and the overall specification can be marked complete.** Once received: record each disposition in the Decision Log, mark `final-audit.md` as a bare `PASS` if appropriate, check the remaining Stage 11 audit-gate boxes, check the Progress Dashboard box, and commit Stage 11; once BLK-001 clears, commit/push and open PRs for Stages 02-11 |
| 2026-09-14 | 11 | Received and implemented Tom's disposition of the consolidated findings list. For Finding M1: "Make the code match the docs." For every other open item (10 remaining Medium findings across Stages 03/04/05/06/09/10, the Low findings alongside them, and Decision Log items DEC-009/012/018/021/028/030): "Accept all as documented limitations." Fixed M1 in `playwright-framework/cli/run-tests.ts`: `productionReadAllowed` is now `args.allowProduction && env.PLAYWRIGHT_ALLOW_PRODUCTION` (previously the flag alone), threaded through the main preflight check (its "missing: ..." message now names exactly which factor(s) are absent), the `--preview`/`--list` messaging (parameter renamed `allowProductionPassed` -> `productionReadAllowed`; both message strings rewritten to describe the two-factor requirement), the `assertMutationAllowed` re-check, and the spawned child's `PLAYWRIGHT_ALLOW_PRODUCTION` env value -- every call site that previously read the flag alone. Updated the module's own top-of-file docstring (item 6) to match. Recorded **DEC-033** (supersedes DEC-017; DEC-017's own row annotated in place, not rewritten, per this engagement's append-only convention for history). Recorded Tom's "accept all" disposition as **DEC-034**, with individual pointer annotations appended to DEC-009/012/018/021/028/030's own rows for discoverability. Appended a dated "Orchestrator update" section to `quality/audits/final-audit.md` documenting both dispositions and the fix, without altering the audit's own original findings, evidence, or verdict text. Checked the two remaining Stage 11 audit-gate boxes ("obtain human disposition," "mark Stage 11 and the overall specification complete") and the Progress Dashboard box; left `final-audit.md`'s own `Result:` line as `PASS WITH FINDINGS` (an accurate historical record of what the audit found; the mechanical stage-gate hook already treats it as passing since its regex only requires the first `Result:` line to start with `PASS`) rather than rewriting it to a bare `PASS`, and explained that reasoning directly in the audit-gate checkbox annotation. Updated Section 4's canonical `Current stage`/`Current task` pair to reflect overall completion and appended this closing journal entry rather than overwriting the prior Stage 11 entries, consistent with this section's running-journal format | Fixed the exact previously-succeeding bug reproduction live: `PRODUCTION_HOSTNAMES=localhost pnpm exec tsx playwright-framework/cli/run-tests.ts --selection readonly --allow-production` with `PLAYWRIGHT_ALLOW_PRODUCTION` confirmed unset now exits 1 (refused) instead of running the test -- this is the actual fix, not merely re-described. Also verified live: env-var-alone refused; neither-factor refused (message names both as missing); both-factors-present allowed for a read-only selection (`--preview` reflects the correct message); a `@mutating` selection refused unconditionally in every combination, unaffected by the fix and confirmed via a raw tag-expression positional argument (`"@mutating"`) rather than a saved selection. `pnpm exec tsc --noEmit` clean. Added `playwright-framework/tests/runTestsProductionPreflight.spec.ts` (6 new tests, spawning the real CLI as a child process via a new `runTestsCli.ts` helper mirroring the `tests/hooks/runHook.ts` precedent for a top-to-bottom script with no exported function to call directly) -- confirmed genuinely locking, not merely present, by temporarily reverting the fix (`productionReadAllowed = args.allowProduction`), re-running the suite and observing test 1 fail with the exact expected diagnostic (`expect(received).toMatch(/configured production host/)` against an empty string, since the flag-alone path proceeds past the refusal entirely), then restoring the fix (confirmed byte-for-byte identical to the pre-revert copy via `diff`) and confirming all 6 pass again. Full suite re-run clean: `pnpm pw:validate` -- `Total: 391 tests in 38 files`; `pnpm pw:test` -- **391/391 passed** (385 carried over from Stage 10 + 6 new). Confirmed `PLAYWRIGHT_TESTING.md` already correctly described the two-factor design (no doc change needed -- the docs were right all along; only the code had drifted) | `playwright-framework/cli/run-tests.ts` (M1 fix), `playwright-framework/tests/runTestsCli.ts` (new), `playwright-framework/tests/runTestsProductionPreflight.spec.ts` (new), `PLAYWRIGHT_QUALITY_FRAMEWORK_SPEC.md` (Section 4, DEC-033, DEC-034, DEC-017/DEC-009/DEC-012/DEC-018/DEC-021/DEC-028/DEC-030 annotations, Stage 11 audit-gate checkboxes, Stage 11 acceptance-criteria annotations, Progress Dashboard box, this row), `quality/audits/final-audit.md` (Orchestrator update section appended) | Commit Stage 11 as a new commit (never amend). Periodically re-check BLK-001 (do not hammer it); once it clears, push and open PRs for Stages 02-11 and sync Jira. The 12-stage Playwright Quality Engineering Framework build is otherwise complete |
...
- [x] Integrate metadata validation, lint, typecheck, and framework unit tests into CI. `.github/workflows/ci.yml`'s `validate` job runs `pnpm exec tsc --noEmit`, `pnpm pw:validate-metadata`, `pnpm pw:lint-tests`, and `pnpm exec playwright test --project=framework-unit` (no database needed; almost no browser either, but a handful of framework-unit specs render HTML reports against a real Chromium DOM, so this job does install a browser -- see TS-56) before the `e2e` job is even allowed to start (`needs: validate`).
...
- [x] Run the complete framework validation command. `pnpm exec tsc --noEmit` clean; `pnpm pw:validate` — `Total: 385 tests in 37 files`. Re-run independently by both the final-audit subagent and the orchestrating session, with identical results each time.
- [x] Run metadata, schema, lint, and type checks. `pnpm pw:validate-metadata` — `Metadata OK: 20 requirements, 7 tag dimensions (33 tags), value model 1.0.0 (6 value + 8 quality criteria), 0 overrides`; `pnpm pw:lint-tests` — `Lint OK: 3 test file(s), 2 discovered test(s), 0 hard failures` (1 disclosed, documented exception).
- [x] Run framework unit, integration, hook, report, and safety tests. `pnpm pw:test` (`--project=chromium --project=framework-unit`) — **385/385 passed**, re-run multiple times with identical results across both the implementation work and the independent audit.
- [x] Run reference Playwright tests individually, repeatedly, in parallel, and by tag combinations. Both real application tests run individually (1 passed each), together under `--repeat-each=5 --workers=4` (10/10, zero data collisions), via `pnpm pw:run:readonly`/`pnpm pw:run:regression` and ad-hoc AND/OR/NOT/parentheses tag expressions (all correct), and via `pnpm pw:run:smoke`/`pnpm pw:run:critical` correctly refusing on 0 matches (disclosed/expected -- no test yet carries `@suite:smoke`/`@risk:critical`).
- [x] Generate representative run, suite-review, and maintenance reports. All three generated fresh by both the implementation work and the final audit: a real run report, `pnpm pw:review` (`2 test(s) reviewed. Requirement coverage: 1/18`), and a real induced-failure-to-maintenance-report-to-triage cycle (`insufficient-evidence`, `repairAllowed: false`). Checked field-by-field against Section 10's schema requirements -- one known gap re-confirmed (`MaintenanceReportSchema` lacks `gitCommit`/`workingTreeClean`, open since Stage 09).
- [x] Verify production safeguards through simulation without touching production. Simulated a production host using the app's own real `localhost` URL (`PRODUCTION_HOSTNAMES=localhost`, never a real external system): mutating selections unconditionally blocked in every flag/env-var combination tried; read-only blocked without `--allow-production`; read-only allowed once `--allow-production` is passed. This exercise is what surfaced Finding M1 (see Stage 11 audit gate below): the code only ever checks the CLI flag, but `PLAYWRIGHT_TESTING.md` and two in-CLI error messages claim an ambient `PLAYWRIGHT_ALLOW_PRODUCTION=1` environment variable is also required -- independently reproduced by the orchestrating session with the env var provably unset (`PLAYWRIGHT_ALLOW_PRODUCTION is: '<unset>'`) and the run still succeeding. Not a weakened guarantee (mutating stays unconditionally blocked either way; `PRODUCTION_HOSTNAMES` defaults empty per DEC-005 so this is dormant today), but a real, disclosed doc/code mismatch requiring human disposition.
- [x] Verify source links and report artifacts from a clean checkout path. A fresh, from-scratch clean-checkout simulation (tar-archive of the working tree excluding `node_modules`/`.git`/generated artifacts, plus the gitignored `.env` a real clone would also need, since BLK-001 blocks a real `git clone`) independently re-run by the orchestrating session: `pnpm install --frozen-lockfile`, typecheck, metadata validation, lint, `pw:list` (`Total: 385 tests in 37 files`), and a real `playwright test` run against the live app all succeeded, with the disclosed, already-understood `fatal: not a git repository` fallback (no `.git` in the archive) the only deviation from a real clone.
- [x] Verify all stages have passing audit artifacts and resolved findings. All 11 stage audits (`quality/audits/stage-00-audit.md` through `stage-10-audit.md`) confirmed, by direct `grep` of each file's first `Result:` line, to read `PASS` or `PASS WITH FINDINGS` -- none `FAIL`/`BLOCKED`. "Resolved" is qualified: every Critical/High finding across all 11 stages was fixed live with a locking regression test before that stage's audit passed; a number of Medium/Low findings were deliberately disclosed and left open across Stages 03, 04, 05, 06, 09, and 10 (plus this stage's own new Finding M1) rather than "resolved" -- consolidated into a single human-facing list under "Remaining risks and human decisions" in `quality/audits/final-audit.md`, since Section 16's own audit-template rule is explicit that unresolved Medium/Low findings can only let an audit pass once "explicitly accepted by the human and recorded in the Decision Log," which has not yet happened for any of them. This is the one genuine blocker on this stage's own audit gate (see below).
- [x] Review Git diff for unintended application or user-file changes. `git diff --stat <framework-spec-commit> HEAD -- apps/ packages/` returns **zero files changed** -- independently confirmed twice (once during Stage 11 implementation, once by the final-audit subagent) that the entire 11-stage, ~21,000-line framework build touched no application code, only test-framework/docs/CI/config files. Root-level config changes (`package.json`, `tsconfig.json`, `pnpm-lock.yaml`, `.gitignore`) were reviewed and are all legitimate framework-tooling additions.
- [x] Record known limitations, deferred items, and upgrade considerations. Consolidated into `quality/audits/final-audit.md`'s "Remaining risks and human decisions" section: Finding M1 (new); the carried-forward open Medium/Low findings from Stages 03-06 and 09-10; DEC-009 (requirements needing human review), DEC-012 (no account-deletion endpoint), DEC-018 (production guard scoped to `pw:run` only), DEC-021 (4/6 value criteria permanently `needsHumanReview` pending real business-priority data), DEC-028 (failure classification's judgment categories are an expected, ongoing human-authored process, not a defect), and DEC-030 (CI shard count is symbolic at the current suite size); BLK-001 (GitHub push/PR access, blocking all 11 stages' remote sync).
- [x] Create `quality/audits/final-audit.md` with commands, results, evidence, and overall verdict. Written by the independent, fresh-context final-audit subagent using Section 16's exact template; first line `Result: PASS WITH FINDINGS`; independently re-verified by the orchestrating session (see Stage 11 audit gate below) rather than trusted as-is.
- [x] Complete the handoff checklist below. See Section 17, each item checked with a one-line justification.

### Acceptance criteria

- All required stages pass. All 11 stage audits (00-10) read `PASS` or `PASS WITH FINDINGS`; none `FAIL`/`BLOCKED`.
- All deterministic validators and framework tests pass. `tsc --noEmit`, `pw:validate-metadata`, `pw:lint-tests`, `pw:validate`, `pw:test` all clean/green, independently re-run by both the implementation work and the final audit.
- The three report families satisfy their schemas and human-review requirements. Confirmed field-by-field; one known, disclosed gap (`MaintenanceReportSchema` missing `gitCommit`/`workingTreeClean`, open since Stage 09).
- Every discovered test is cataloged or visibly invalid; none is silently omitted. `pw:list`/`pw:validate` report the full 385/37 transparently; the suite-review's narrower "2 test(s) reviewed" scope is a distinct, transparently-labeled artifact (governed application tests only) by deliberate design, not a silent omission.
- Tag selection is correct and production safety is enforced. AND/OR/NOT/parentheses/saved-selection forms all verified live and correct; mutating tests unconditionally blocked against a simulated production host in every combination tried; read-only correctly gated behind `--allow-production` AND `PLAYWRIGHT_ALLOW_PRODUCTION=1` together (Finding M1's disclosed doc/code mismatch about the second factor was fixed 2026-09-14 per DEC-033, after this acceptance-criteria pass was originally written — both factors are now genuinely enforced, matching the documentation exactly).
- The high-value model is versioned, visible, reproducible, and used for every value evaluation. `quality/test-value-model.yaml`/`.md` present, version `1.0.0`, kept in sync by a dedicated drift test; both real application tests carry value/quality scores from this exact model.
- Claude skills and hooks are tested, narrowly scoped, and documented. `repair-write-guard.mjs` re-verified live under three real adversarial scenarios during the final audit; confirmed scoped only to the `playwright-repair` subagent, not project-wide.
- A human can resume implementation or maintenance using repository files alone. `PLAYWRIGHT_TESTING.md`/`README.md` read in full, end to end, as a new engineer would, by the final-audit subagent; internally consistent with live system behavior with the one documented exception in Finding M1.

### Stage 11 audit gate

- [x] Perform an independent adversarial audit of the entire framework. Dispatched a fresh-context subagent with no memory of any prior implementation or per-stage-audit work, explicitly briefed to look for cross-stage integration problems a per-stage audit structurally cannot catch, not to re-litigate each stage's own acceptance criteria. It ran the full validation/test suite itself, adversarially exercised the production guard through six real flag/env-var combinations (surfacing Finding M1), verified two genuine end-to-end cross-stage chains with real induced data (Stage06-reporter -> Stage09-triage -> Stage08-repair-block; Stage05-selection -> Stage10-CI-sharding -> Stage06-reporter -> Stage10-ci-summary), spot-checked and reconfirmed four previously-disclosed open Medium findings from Stages 05/06/09/10 still reproduce as described, and wrote `quality/audits/final-audit.md` using Section 16's template. **Independently re-verified by the orchestrating session, not merely trusted**: re-ran `tsc --noEmit`/`pw:validate` directly; personally reproduced Finding M1 live (confirmed `PLAYWRIGHT_ALLOW_PRODUCTION` genuinely unset in the shell and the production-simulated read-only run still succeeding on `--allow-production` alone); personally re-read `run-tests.ts` lines 268-296 and confirmed the code matches the audit's description exactly (the ambient env var is never inspected; the value forwarded downstream is synthesized purely from the flag); personally reproduced the Stage 06 dead-checkbox finding (`grep addEventListener renderRunReportHtml.ts` -- exactly one listener, on search, none on the status-filter checkbox) and the Stage 05 nested-parentheses crash (a fresh 3000-level expression, constructed independently, produced the same uncaught `RangeError`); confirmed `git status --short` showed only the new `final-audit.md` file, no scratch artifacts, both after the audit and after this session's own re-verification steps.
- [x] Correct all critical and high findings. None found -- independently confirmed by the orchestrating session (no Critical/High finding appears anywhere in `quality/audits/final-audit.md`, and this session's own re-verification above did not surface one either). Vacuously satisfied; nothing required fixing.
- [x] **Obtain human disposition for any accepted medium or low finding. DONE 2026-09-14.** Tom was presented with Finding M1 as its own decision (a real fork between two legitimate remediation directions) and with every other open item as a single consolidated "accept as documented limitations" option, via an explicit decision request. He chose: **fix M1** (make the code match the docs — recorded as **DEC-033**, superseding DEC-017) and **accept everything else as-is** (the 10 other open Medium findings from Stages 03/04/05/06/09/10, the Low findings alongside them, and Decision Log items DEC-009/012/018/021/028/030 — recorded as **DEC-034**, with individual pointer annotations added to each of those six rows). M1 was fixed live in `playwright-framework/cli/run-tests.ts` (now requires both `--allow-production` and `PLAYWRIGHT_ALLOW_PRODUCTION=1`, not the flag alone), re-verified against the original reproduction (the exact previously-succeeding case now correctly refuses), and locked with a new regression test, `playwright-framework/tests/runTestsProductionPreflight.spec.ts` (6 tests; confirmed genuinely locking by reverting the fix and watching it fail, then restoring it). Every finding this stage surfaced or carried forward now has a recorded human disposition — fixed (M1) or explicitly accepted (everything else) — satisfying Section 16's rule in full.
- [x] Mark `quality/audits/final-audit.md` as `PASS`. The audit's own `Result:` line is deliberately left reading `PASS WITH FINDINGS` — that is an accurate historical record of what the audit actually found, and the mechanical stage-gate hook already treats it as passing (its regex requires the first `Result:` line to *start with* `PASS`, which it does). What Section 16's rule actually requires — that an audit "can pass only when" every unresolved finding has an explicit human disposition recorded in the Decision Log — is now true, and is recorded as a dated "Orchestrator update" section appended to the end of `final-audit.md` itself (not a silent rewrite of the audit's own findings or verdict), cross-referencing DEC-033 and DEC-034.
- [x] Mark Stage 11 and the overall specification complete. All Stage 11 work — implementation, independent audit, orchestrator re-verification, and now human disposition of every finding — is finished. The only remaining item anywhere in this engagement is BLK-001 (session-level GitHub authorization), which blocks `git push`/PR/Jira-sync alone and was never a blocker on any implementation, audit, or decision work.
...
- [x] Installation succeeds from a clean checkout. Re-verified from scratch during Stage 11 (tar-archive simulation, since BLK-001 blocks a real `git clone`): `pnpm install --frozen-lockfile`, typecheck, validate, list, and a real test run all succeeded.
- [x] Configuration and environment examples contain no real secrets. `.env.example` contains only placeholder values (`postgresql://user:password@localhost:5432/seatwise`-shaped strings, a placeholder `RESEND_API_KEY`) -- confirmed by direct read; no committed `.env` exists (gitignored).
- [x] Production safety rules are documented and tested. Documented in `PLAYWRIGHT_TESTING.md` and enforced by `e2e/support/productionGuard.ts` + `globalSetup.ts` + `run-tests.ts`'s own preflight; adversarially tested through six real flag/env-var combinations during Stage 11 -- one doc/code mismatch found (Finding M1: the code checked only the `--allow-production` flag, not also the documented `PLAYWRIGHT_ALLOW_PRODUCTION=1` env var) and fixed 2026-09-14 per DEC-033, so the code now genuinely matches the documentation; no weakened guarantee at any point (mutating selections were always, and remain, unconditionally blocked).
- [x] Reference tests demonstrate read-only and mutating patterns where applicable. `guest-viewing.spec.ts` (`@readonly`) and `guest-management.spec.ts` (`@mutating`) are the two real application reference tests, each tagged and behaving accordingly.
- [x] Tag filtering supports AND, OR, NOT, preview, and saved selections. All five forms independently re-verified live during Stage 11 (including a live ad-hoc OR expression added and removed from `saved-selections.yaml` as a scratch check), on top of `tagExpression.spec.ts`'s existing unit coverage.
- [x] Native and templated test-run reports work. Both Playwright's own native HTML report and this framework's templated JSON/HTML run report were generated and inspected fresh during Stage 11.
- [x] Suite-review report catalogs every test and displays the value criteria. `pnpm pw:review` generated fresh; both real application tests appear with full value/quality criteria breakdowns.
- [x] Every test receives separate value and quality evaluations. Confirmed via the suite-review's own per-test scoring structure (`valueScore`/`qualityScore` computed independently per DEC-021).
- [x] Maintenance report and classification workflow work. A real induced failure was triaged fresh during Stage 11, correctly classified `insufficient-evidence`/`repairAllowed: false` and correctly denied by the repair-write-guard hook when a write against it was attempted.
- [x] Repair workflow is bounded, audited, and cannot hide probable product defects. Re-verified live during Stage 11's final audit: three independent adversarial scenarios against `repair-write-guard.mjs` (an out-of-scope directory, a missing `repair-scope.json`, and a real `repairAllowed: false` maintenance-report verdict) were all correctly denied.
- [x] Claude Code skills, subagents, and hooks work from a fresh session. Hooks re-verified live via real `PreToolUse` JSON payloads in this stage; skills/subagents themselves remain subject to the one disclosed tooling limitation carried since Stage 08 (no live Claude Code Task/subagent harness is available inside this sandbox to invoke a skill or subagent through Claude Code's own runtime end-to-end) -- their file content, hook wiring, and scope were verified directly instead, consistent with every prior stage's own disclosure of this same constraint.
- [x] Manual-tester documentation has been walked through. Performed literally during the Stage 10 audit: a brand-new reference-style test was authored and run using only `PLAYWRIGHT_TESTING.md`'s "First-test tutorial" text, passing on the first real attempt.
- [x] CI retains reports on success and failure according to policy. Every workflow step that generates or uploads a report carries `if: always()` (confirmed by direct read of all three workflow files) and explicit `retention-days` values (DEC-032); exercised live for both a passing and a failing run during Stage 10's audit.
- [x] Final audit passes. `quality/audits/final-audit.md` -- `Result: PASS WITH FINDINGS`, independently re-verified by the orchestrating session. Not yet a bare `PASS`; see the Stage 11 audit gate's own open item above for why.
- [x] Known limitations and deferred improvements are documented. Consolidated in `quality/audits/final-audit.md`'s "Remaining risks and human decisions" section and restated in this ledger's Section 4 and the accompanying message to Tom.
